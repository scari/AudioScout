...
int recvsnd(void *rcvskt, void *sndskt, size_t *msg_size, int64_t *more, size_t *more_size,\
	...
	*data = malloc(*msg_size);
	...
	zmq_msg_send(&topic_msg, sndskt, ZMQ_SNDMORE);
	zmq_msg_close(&topic_msg);
    }
    int res = zmq_msg_send(&msg, sndskt, (more) ? ZMQ_SNDMORE : 0);
...
int recieve_msg(void *skt, size_t *msg_size, int64_t *more, size_t *more_size,\
    ...
    zmq_msg_t msg;
    zmq_msg_init(&msg);
    int res = zmq_msg_recv(&msg, skt, 0);
    if (msg_size){
	*msg_size = zmq_msg_size(&msg);
    }
    if (data) {
...
int send_empty_msg(void *skt){
    ...
    int res = zmq_msg_send(&msg, skt, 0);
    zmq_msg_close(&msg);
    return res;
}
...
int sendmore_msg_vsm(void *skt, void *data, size_t size){
    ...
    zmq_msg_init_size(&msg, size);
    memcpy(zmq_msg_data(&msg), data, size);
    int res = zmq_msg_send(&msg, skt, ZMQ_SNDMORE);
    zmq_msg_close(&msg);
    return res;
}
